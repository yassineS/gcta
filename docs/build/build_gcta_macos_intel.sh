#!/bin/bash
# build_gcta_macos_intel.sh
# Builds GCTA static binary on macOS Intel (x86_64)
...
echo -e "${BLUE}Building GCTA static binary for macOS Intel (x86_64)${NC}"
...
BUILD_DIR="$GCTA_SRC/build_macos_intel"
mkdir -p "$BUILD_DIR"
cd "$BUILD_DIR"

echo -e "${BLUE}Configuring GCTA with CMake for macOS Intel...${NC}"
cmake -DCMAKE_BUILD_TYPE=Release \
      -DCMAKE_SYSTEM_NAME=Darwin \
      -DCMAKE_SYSTEM_PROCESSOR=x86_64 \
      -DCMAKE_OSX_ARCHITECTURES=x86_64 \
      -DCMAKE_CXX_FLAGS="-mmacosx-version-min=10.13" \
      -DCMAKE_C_FLAGS="-mmacosx-version-min=10.13" \
      .. > /dev/null 2>&1
