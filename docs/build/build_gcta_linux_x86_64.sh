#!/bin/bash
# build_gcta_linux_x86_64.sh
# Builds GCTA static binary on Linux x86_64
...
echo -e "${BLUE}Building GCTA static binary for Linux x86_64${NC}"
echo "GCTA source: $GCTA_SRC"
echo "Build root: $GCTA_BUILD_ROOT"

# Verify environment
if [ -z "$MKLROOT" ]; then
    echo "ERROR: MKLROOT not set"
    exit 1
fi

if [ -z "$EIGEN3_INCLUDE_DIR" ]; then
    echo "ERROR: EIGEN3_INCLUDE_DIR not set"
    echo "Solution: Run build_deps_linux_x86_64.sh first"
    exit 1
fi
...
BUILD_DIR="$GCTA_SRC/build_linux_x86_64"
mkdir -p "$BUILD_DIR"
cd "$BUILD_DIR"
